...
class Board:
    ...
    def create_tile_lines(self, tile, pos, players):
        ...
        name = self.fit_text(tile["name"], 9)
        type_display = tile['type'][:7] if tile['type'] != 'community_chest' else "C.Chest"
        price_str = f"${tile['price']}" if tile.get('price', 0) > 0 else ""

        lines = []
        lines.append(color + "┌" + "─"*9 + "┐" + reset)
        lines.append(color + f"│{str(pos).center(9)}│" + reset)
        lines.append(color + f"│{name}│" + reset)
        lines.append(color + f"│{type_display.center(9)}│" + reset)
        lines.append(color + f"│{price_str.center(9)}│" + reset)
        lines.append(color + f"│[{player_str}]".ljust(10) + "│" + reset)
        lines.append(color + "└" + "─"*9 + "┘" + reset)
        return lines
    ...
    def render_board(self, players: Dict[str, int] = None):
        ...
        # Cạnh trên (20-30) - đảo ngược để hiển thị đúng
